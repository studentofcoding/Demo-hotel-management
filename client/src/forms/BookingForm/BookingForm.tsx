...
export type BookingFormData = {
    ...
    hotelId: string;
    totalCost: number;
    paymentIntentId: string;   //to find the payment is valid or not
};
...
const BookingForm = ({ currentUser,paymentIntent }: Props) => {
    ...
    const {hotelId} = useParams();   //this will get the hotelId from the url
    ...
    const onSubmit = async (formData: BookingFormData) => {
        if(!stripe  || !elements) return;   //this will return if there is no stripe
        ...
        if(result.paymentIntent?.status === "succeeded"){
            bookRoom({ ...formData, paymentIntentId: result.paymentIntent.id });   //this will book the room
        }
    }
            ...
            <div className="space-y-2">
                ...
                <div className="bg-blue-200 p-4 rounded-md">
                    <div className="font-semibold text-lg">
                        Total Cost: £{paymentIntent.totalCost.toFixed(2)}   {/* this will show the total cost,toFixed(2) to two decimal places */}
                    </div>
